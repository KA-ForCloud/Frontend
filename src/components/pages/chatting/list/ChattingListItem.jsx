...
import {connect, getRooms, subscribe,getChattingListItemInfo,updateLastRead} from '../../../../services/ChattingService';
...
function ChattingListItem(props) {
    ...
    useEffect(() => {
        ...
        getChattingListItemInfo(item.chattingId).then((response)=>{
            console.log("CHATTINGLISTITEM-----",response.data.result.last);
            setNumberOfChattings(response.data.result.number-1);
            setNewChat(response.data.result.last);
            console.log("newCHatdfkaljf",newChat);

            setCheck(true);
            console.log("numberOfChattings-----",numberOfChattings);
