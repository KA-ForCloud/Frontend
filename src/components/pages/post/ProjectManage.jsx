  ...
  return (
    <>
      ...
      <div className="mx-auto w-9/12 my-4 px-4 border-4 border-sky-200 rounded-2xl">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 md:grid-cols-2 gap-y-4 overflow-auto"> {myProjectList && <PostList postList = {myProjectList} type={"ongoing"}/>} </div>
          <hr className="h-px my-4 border-2 border-indigo-100"></hr>

          <p className="text-2xl font-bold text-gray-900 m-4">완료된 프로젝트</p>
          <div className="mb-4 grid grid-cols-1 xl:grid-cols-3 md:grid-cols-2 gap-y-4 overflow-auto"> {myProjectList && <PostList postList = {myProjectList} type={"completed"}/>}  </div>
      </div>
    </>
  )
}
