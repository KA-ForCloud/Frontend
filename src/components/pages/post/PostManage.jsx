    ...
    const PostList = (num) => {
        ...
        return modifiedPostList.map((item,idx) => (
          <div key ={idx} className="min-w-max rounded-2xl mx-2 border flex-column hover:bg-sky-50 transition cursor-pointer"
            ...
            <h3 className="mx-5 my-2 text-dark font-weight-bold">프로젝트 제목: {item.post_name}</h3>
            <h3 className="mx-5 my-2 text-dark font-weight-bold">모집기한: {item.end_time}</h3>
            ...
            <div className="flex my-2">
              ...
              <h3 className="mx-auto text-dark font-weight-bold">조회수: {item.views}회</h3>
            </div>
          </div>
        ))
      }
