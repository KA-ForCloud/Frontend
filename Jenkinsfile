        ...
        stage('Deploy to dev') {
          steps {
                sshagent(credentials: ['kic_key']) {
                    ...
                    sh "ssh -o StrictHostKeyChecking=no centos@210.109.60.60 -p 10001 'docker run -d --name front -P exposed-ports lmslmsms0616/teamchat_front:${currentBuild.number}'"
                    echo "docker run"
                    ...
                    sh "ssh -o StrictHostKeyChecking=no centos@210.109.60.60 -p 10002 'docker run -d --name front -P exposed-ports lmslmsms0616/teamchat_front:${currentBuild.number}'"
